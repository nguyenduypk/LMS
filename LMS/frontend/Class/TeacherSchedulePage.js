...
import Header from '../src/components/teacher/Header';
...
export default function TeacherSchedulePage() {
  ...
  // Mock class information - in real app this would come from API
  const classInfo = {
    name: 'Lớp học mẫu',
    code: classId || 'CLASS001',
    teacher: 'Nguyễn Văn A'
  };
                    ...
                    return (
                      ...
                      >
                        <div className="teacher-event-card-header">
                          ...
                          <span className="teacher-event-status">Đang diễn ra</span>
                        </div>
                        <div className="teacher-event-class">{event.className}</div>
          ...
          <tbody>
            {Object.entries(WEEKLY_SCHEDULE).map(([dayKey, events]) => 
              events.map((event, eventIndex) => {
                ...
                return (
                  <tr key={`${dayKey}-${eventIndex}`} className="teacher-schedule-row">
                    <td className="teacher-class-name">{event.className}</td>
                    <td className="teacher-room-name">{event.room}</td>
                    ...
                    <td className="teacher-status">
                      <span className={`teacher-status-badge ${status.toLowerCase().replace(/\s+/g, '-')}`}>
                        {status}
                      </span>
                    </td>
  ...
  return (
    <div className="teacher-schedule-container" onClick={(e) => console.log('Container clicked:', e.target)}>
      <Header />
          ...
          <div className="teacher-tooltip-header" style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:12}}>
            <div style={{display:'flex',alignItems:'center',justifyContent:'flex-start',flex:1,marginLeft:'12px'}}>
              {['Google Meet', 'Zoom', 'Teams'].includes(hoveredEvent.type) && (
                <button className="teacher-main-action-btn join-room-btn" style={{background:'#1976d2',color:'#fff',fontWeight:700,padding:'10px 20px',borderRadius:6,border:'none',fontSize:14,cursor:'pointer'}} onClick={() => {
                  let roomUrl = '';
          ...
          <div className="teacher-popup-header" style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:12}}>
            <div style={{display:'flex',alignItems:'center',justifyContent:'flex-start',flex:1,marginLeft:'12px'}}>
              {['Google Meet', 'Zoom', 'Teams'].includes(selectedEvent.type) && (
                <button className="teacher-main-action-btn join-room-btn" style={{background:'#1976d2',color:'#fff',fontWeight:700,padding:'10px 20px',borderRadius:6,border:'none',fontSize:14,cursor:'pointer',marginRight:12}} onClick={() => {
                  let roomUrl = '';
