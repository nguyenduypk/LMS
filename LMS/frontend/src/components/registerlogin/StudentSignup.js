...
function StudentSignup() {
  ...
  const validate = () => {
    ...
    if (!form.className.trim()) newErrors.className = 'Vui lòng nhập lớp đang học';
    return newErrors;
  };
  ...
  const handleSubmit = e => {
    ...
    if (Object.keys(newErrors).length === 0) {
      ...
      console.log('Đăng ký học sinh thành công:', form);
    }
  };
    ...
    <>
      ...
      <div className="role-select-container">
        <form className="teacher-signup-form" onSubmit={handleSubmit} autoComplete="off">
          <h2 className="role-select-title">Đăng ký tài khoản học sinh</h2>
          ...
          <div className="signup-field">
            ...
            />
            {errors.className && <div className="signup-error">{errors.className}</div>}
          </div>
          <button className="btn btn-primary signup-btn" type="submit">Đăng ký</button>
          <div className="role-switch-link" style={{ marginTop: 20 }}>
            Bạn là giáo viên? <Link to="/signup/teacher">Đăng ký tại đây</Link><br />
