...
function StudentClassPage() {
  ...
  const [visibleClasses, setVisibleClasses] = useState(() => {
    const data = localStorage.getItem('student_visible_classes');
    return data ? JSON.parse(data) : sampleClasses;
  });
  const [pendingClasses] = useState(() => {
    const data = localStorage.getItem('student_pending_classes');
    if (data) {
  ...
  const [hiddenClasses, setHiddenClasses] = useState(() => {
    const data = localStorage.getItem('student_hidden_classes');
    if (data) {
      const arr = JSON.parse(data);
      if (Array.isArray(arr) && arr.length > 0) return arr;
    }
  ...
  const [trashedClasses, setTrashedClasses] = useState(() => {
    const data = localStorage.getItem('student_trashed_classes');
  ...
  useEffect(() => {
    localStorage.setItem('student_visible_classes', JSON.stringify(visibleClasses));
  }, [visibleClasses]);
  useEffect(() => {
    localStorage.setItem('student_hidden_classes', JSON.stringify(hiddenClasses));
  }, [hiddenClasses]);
  useEffect(() => {
    localStorage.setItem('student_trashed_classes', JSON.stringify(trashedClasses));
  }, [trashedClasses]);
  ...
  const handleMenuAction = (action, id) => {
    ...
    if (action === 'join') {
      ...
      if (cls) {
        navigate(`/class/${cls.code}/announcement`);
      }
      return;
    }
    if (action === 'hide') {
  ...
  const handleTrashConfirm = () => {
    ...
    if (cls) {
      ...
      localStorage.setItem('student_visible_classes', JSON.stringify(newVisible));
      localStorage.setItem('student_hidden_classes', JSON.stringify(newHidden));
      localStorage.setItem('student_trashed_classes', JSON.stringify(newTrashed));
  ...
  const tabData = [
    ...
    { key: 'pending-classes', label: `Lớp đang chờ (${pendingClasses.length})` },
